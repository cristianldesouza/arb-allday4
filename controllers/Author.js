...
class Author {
	async handleLang(lang, slug, page, request, env, ctx) {
		let authorData = await Requests.getAuthorData(
			slug,
			lang,
			constants.DOMAIN,
			page,
		...
		let replacesMap = {
			...
			'{{author.posts}}': Template.authorPostsLoop(
				authorData.posts,
				lang
			),
			'{{pagination}}': Template.renderPagination(
				authorData.pagination,
				lang,
				`a/${slug}`
			),
			'{{socialmedia}}': Template.authorSocialMediaLoop(
				author.social_media
			),
		...
		for (let index in replacesMap) {
			content = Template.searchAndReplace(
				content,
				index,
				replacesMap[index]
			);
	...
	async authorsListLang(lang, request, env, ctx) {
		try {
			...
			const authorsListData = await Requests.getAuthorsListData(
				lang,
				request.query
			);
			...
			if (
				...
				authorsListData.authors.length === 0
			) {
				return NotFound.index(lang, request, env, ctx);
			}
					...
					return Template.renderTemplate('authors_card', {
						'author.url':
							lang == constants.LANGUAGES[0]
								? `/a/${author.slug}/`
								: `/${lang}/a/${author.slug}/`,
						'author.img': Util.generateCdnUrl(
							author.avatar,
							400,
							400,
							70
